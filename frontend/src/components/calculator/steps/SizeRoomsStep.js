...
export default function SizeRoomsStep({ value, onNext, onBack }) {
  const [squareFootage, setSquareFootage] = useState(value.squareFootage || 2200);
  const [bedrooms, setBedrooms] = useState(value.bedrooms ?? 3);
  const [fullBathrooms, setFullBathrooms] = useState(value.fullBathrooms ?? 2);
  const [halfBathrooms, setHalfBathrooms] = useState(value.halfBathrooms ?? 0);

  return (
    <StepWrapper
      illustration={<HouseTypeIllustration />}
      title="Size & rooms"
      subtitle="Square footage is the single biggest driver of total cost."
      onBack={onBack}
      onNext={() => onNext({ squareFootage, bedrooms, fullBathrooms, halfBathrooms })}
    ...
    >
      <div style={{ marginBottom: 26 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 4 }}>
          <span>500 sqft</span><span>8,000 sqft</span>
        </div>
        <input
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 28 }}>
        ...
        <div>
          <div style={labelStyle}>Full Bathrooms</div>
          <Stepper value={fullBathrooms} onChange={setFullBathrooms} min={1} max={8} />
        </div>
        <div>
          <div style={labelStyle}>Half Bathrooms</div>
          <Stepper value={halfBathrooms} onChange={setHalfBathrooms} min={0} max={4} />
        </div>
      </div>
    </StepWrapper>
  );
