    ...
    <footer style={{ background: 'var(--primary-dark)', color: '#cfe0d6', padding: '48px 24px 28px', marginTop: 40 }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 32, marginBottom: 32 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
              ...
              <span style={{ fontWeight: 800, fontSize: 17, color: 'white' }}>HomeCostCal</span>
            </div>
            <p style={{ fontSize: 13, lineHeight: 1.7, color: '#a9c2b4', maxWidth: 260 }}>
              A free, data-backed calculator for estimating what it costs to build a new home — by state, size, design, and neighborhood.
            </p>
          </div>
          ...
          <div>
            ...
            <p style={{ fontSize: 12.5, lineHeight: 1.7, color: '#a9c2b4' }}>
              NAHB, U.S. Census Bureau, HomeAdvisor/Angi, RSMeans, and live OpenStreetMap location data.
            </p>
            <a href="mailto:hello@homecostcal.com" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--accent)', marginTop: 10 }}>
              <Mail size={13} /> hello@homecostcal.com
            </a>
          </div>
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 20, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, fontSize: 12, color: '#7f9a8a' }}>
          <span>© {new Date().getFullYear()} HomeCostCal. All rights reserved.</span>
          <span>Estimates are for informational purposes only and are not a bid, appraisal, or guarantee of price.</span>
        </div>
      </div>
    </footer>
  );
