// HomeCostCal — Location intelligence service
//
...
// Both are real, live public datasets — not hardcoded guesses — which is what lets us
// give a genuine "how close is this to a school/store" answer for any US address.

const NOMINATIM_URL = 'https://nominatim.openstreetmap.org/search';
const OVERPASS_URL = 'https://overpass-api.de/api/interpreter';
const USER_AGENT = 'HomeCostCal/1.0 (housing cost estimator; contact via app)';
...
async function getProximityData(lat, lng, radiusMeters = 3200) {
  ...
  for (const el of data.elements || []) {
    ...
    for (const { key, filter } of AMENITY_QUERIES) {
...
/** Suggests a neighborhood pricing tier from the convenience score, for pre-filling the calculator */
